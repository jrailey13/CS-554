...
export const resolvers = {
    ...
    Query: {
        ...
        getSongsByArtistId: async(_, args) => {
            ...
            if (exists) {
                ...
                const artists = await artistCollection();
                // Code snippet inspired from https://stackoverflow.com/questions/6502541/mongodb-query-multiple-collections-at-once
                const songs = await artists.aggregate([{$match: {_id: new ObjectId(args.artistId)}},
                    {$lookup: {from: "albums", localField: "_id", foreignField: "artistId", as: "albums"}},
                    {$project: {"songs": "$albums.songs"}}]).toArray();
                if (!songs[0].songs[0]) {
                    throw new GraphQLError(
                        `No songs in the database for the artist ID of ${args.artistId}`,
                        {
                            extensions: {code: 'NOT_FOUND'}
                        }
                    );
                }
                await client.lPush("songs:" + args.artistId.toString(), JSON.stringify(songs[0].songs[0]));
                await client.expire("songs:" + args.artistId.toString(), 3600);
                return songs[0].songs[0];
    ...
    RecordCompany: {
        albums: async (parentValue) => {
            const albums = await albumCollection();
            const albumList = await albums.find({
                recordCompanyId: parentValue._id
            }).toArray();
            return albumList;
        },
        numOfAlbums: async (parentValue) => {
    ...
    Mutation: {
        addArtist: async (_, args) => {
            ...
            console.log(args.dateFormed)
            if (!helpers.validDate(args.dateFormed)) {
            ...
            if (!helpers.validMembers(args.members)) {
                throw new GraphQLError('Members array is invalid', {extensions: {code: 'BAD_USER_INPUT'}})
            }
        ...
        editArtist: async (_, args) => {
            ...
            if (!args.name && !args.dateFormed && !args.members) {
            ...
            if (args.dateFormed) {
                ...
                artistObj.dateFormed = args.dateFormed;
            }
        ...
        addCompany: async (_, args) => {
            ...
            if (!helpers.validFoundedYear(args.foundedYear)) {
        ...
        editCompany: async (_, args) => {
            ...
            if (!args.name && !args.foundedYear && !args.country) {
                throw new GraphQLError('At least one field to edit must be provided', {extensions: {code: 'BAD_USER_INPUT'}})
            }
            ...
            if (args.foundedYear) {
                ...
                companyObj.foundedYear = args.foundedYear;
            }
        ...
        addAlbum: async (_, args) => {
            ...
            if (!helpers.validId(args.recordCompanyId.toString())) {
                throw new GraphQLError('Record company ID is invalid', {extensions: {code: 'BAD_USER_INPUT'}})
            }
            const recordCompanies = await recordCollection();
            let recordCompany = await recordCompanies.findOne({_id: new ObjectId(args.recordCompanyId)});
            if (!recordCompany) {
                throw new GraphQLError(
                    `Could not find album with a record company ID of ${args.recordCompanyId}`,
                    {
                        extensions: {code: 'BAD_USER_INPUT'}
                    }
                );
            ...
            const newAlbum = {
                ...
                recordCompanyId: new ObjectId(args.recordCompanyId),
                songs: []
            }
            const albums = await albumCollection();
            ...
            await recordCompanies.updateOne({_id: new ObjectId(args.recordCompanyId)}, {$push: {albums: new ObjectId(newAlbum._id)}});
        ...
        editAlbum: async (_, args) => {
            ...
            if (!args.title && !args.releaseDate && !args.genre && !args.songs && !args.artistId && !args.recordCompanyId) {
                throw new GraphQLError('At least one field to edit must be provided', {extensions: {code: 'BAD_USER_INPUT'}})
            }
            ...
            if (args.recordCompanyId) {
                ...
                albumObj.recordCompanyId = new ObjectId(args.recordCompanyId);
            }
            ...
            await recordCompanies.updateOne({_id: new ObjectId(args.recordCompanyId)}, {$push: {albums: new ObjectId(albumToUpdate._id)}});
